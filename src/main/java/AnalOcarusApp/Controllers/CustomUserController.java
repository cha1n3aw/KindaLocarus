...
import static AnalOcarusApp.Constants.Constants.REQUEST_ERROR;
...
public class CustomUserController
{
            ...
            @RequestParam(required = false, name="roles", defaultValue = "USER") List<String> roles)
    {
        ...
        if (authentication != null)
        {
            if (authentication.getAuthorities().stream().anyMatch(c -> c.getAuthority().equals("ADMIN")))
            {
                ...
                extendedUserDetailsRepo.save(new AnalOcarusApp.Models.Users.CustomUser(){{
    ...
    public String Govno()
    {
        List<AnalOcarusApp.Models.Users.CustomUser> users;
        ...
        users = mongoTemplate.find(query, AnalOcarusApp.Models.Users.CustomUser.class, "ExtendedUserDetails");
        return ((AnalOcarusApp.Models.Users.CustomUser)(users.toArray()[0])).getUsername();
    }
    ...
    public ResponseEntity<Response<?>> Blyad()
    {
        ...
        extendedUserDetailsRepo.save(new AnalOcarusApp.Models.Users.CustomUser(){{setUsername("pivo"); setPassword(bCryptPasswordEncoder.encode("vino")); setAuthorities(new ArrayList<GrantedAuthority>(){{add(new SimpleGrantedAuthority("USER")); add(new SimpleGrantedAuthority("ADMIN"));}});}});
